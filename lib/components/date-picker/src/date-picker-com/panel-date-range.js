...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { emit }) {
    ...
    const leftLabel = vue.computed(() => {
      const yearOffset = utils.getDayDiffValue(props.buddhistEra);
    ...
    const timeFormat = vue.computed(() => {
      return utils$1.extractTimeFormat(format);
    });
    const dateFormat = vue.computed(() => {
      return utils$1.extractDateFormat(format);
    });
    const leftPrevYear = () => {
    ...
    const btnDisabled = vue.computed(() => {
      return !(minDate.value && maxDate.value && !rangeState.value.selecting && utils.isValidRange([minDate.value, maxDate.value]));
    });
    ...
    const formatToString = (value) => {
      const template = props.buddhistEra ? utils.getBuddhistEraFormat(format) : format;
      return shared.isArray(value) ? value.map((_) => _.format(template)) : value.format(template);
    };
    const parseUserInput = (value) => {
      if (props.buddhistEra) {
        const formatStr = utils.parseBuddhistEraToChristEra(format);
        return shared.isArray(value) ? value.map((_) => dayjs__default["default"](utils.parseBuddhistEraUserInputToDayjs(_, format), formatStr).locale(lang.value)) : dayjs__default["default"](utils.parseBuddhistEraUserInputToDayjs(value, format), formatStr).locale(lang.value);
      }
...
var DateRangePickPanel = /* @__PURE__ */ pluginVue_exportHelper["default"](_sfc_main, [["__file", "/Users/sontayarungsaard/Documents/work/LandBank/element-plus/packages/components/date-picker/src/date-picker-com/panel-date-range.vue"]]);
